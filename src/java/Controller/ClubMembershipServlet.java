...
public class ClubMembershipServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (studIDStr == null || clubIDStr == null || action == null) {
            response.sendRedirect("studentClub.jsp?error=invalid_request");
            return;
        }
        ...
        try {
            ...
            if ("join".equals(action)) {
                String result = CLUB.addStudentToClub(studID, clubID);
                if ("success".equals(result)) {
                    response.sendRedirect("studentClub.jsp?message=joined_successfully");
                } else {
                    response.sendRedirect("studentClub.jsp?error=" + result.replace(" ", "_"));
                }
            } else if ("leave".equals(action)) {
                boolean success = CLUB.removeStudentFromClub(studID, clubID);
                if (success) {
